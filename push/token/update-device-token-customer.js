module.exports = {
    ...
    fn: async function (inputs, exits) {
        ...
        if (inputs.customerId == undefined) {
            ...
            inputs.id = inputs.customerId;
        }

        var userDataObj = await Customer.findOne({ id: inputs.id });
        if (!userDataObj) {
            return res.notFound({ message: sails.__('User record not found') });
        }

        // delete other user endpoint arn with same token
        var usersWithSameTokenRes = await Customer.find({ deviceToken: inputs.deviceToken, id: { '!=': inputs.id } });
        ...
        if (userDataObj.snsEndPointArn && userDataObj.deviceToken != inputs.deviceToken) {
            ...
            var createArnRes = await sails.helpers.push.createArn('customer', inputs.deviceType, inputs.deviceToken, userDataObj.email);
            var endpointArn = createArnRes.endpointArn;

            // subscribe to endpoint arn
            var snsTopicScriptionArn = "";
            var topicSubscribeRes = await sails.helpers.push.topicSubscribe('customer', inputs.deviceType, endpointArn);
            ...
            await Customer.update({ id: inputs.id }, { snsEndPointArn: endpointArn, snsTopicScriptionArn: snsTopicScriptionArn, deviceType: inputs.deviceType, deviceToken: inputs.deviceToken }).fetch();
